...
def main():
    ...
    with col1:
        ...
        if img_file:
            ...
            st.image(preview_img, caption="Hình ảnh đã cắt", use_column_width=True)
    ...
    with col2:
        ...
        if img_file:
            ...
            # Thực hiện truy vấn và lấy danh sách ảnh kết quả
            ...
            for idx, col in enumerate(retrieved_cols):
                image_path = image_list[retriev[idx]]
                image = Image.open(image_path)
                col.image(image, caption=f"Kết quả {idx + 1}", use_column_width=True)
            ...
            for i, col in enumerate(more_cols):
                for u in range(i + 2, 11, 3):
                    image_path = image_list[retriev[u]]
                    image = Image.open(image_path)
                    col.image(image, caption=f"Kết quả {u + 1}", use_column_width=True)
